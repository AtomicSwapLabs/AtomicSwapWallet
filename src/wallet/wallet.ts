...
// const ELECTRUM_URI = 'electrum.bitaroo.net'
const ELECTRUM_URI = '168.119.33.233'
const ELECTRUM_PORT = 50001;

const WALLET_NAME = 'Atom Swap Wallet';
...
export class Wallet {
    ...
    async connectToElectrum() {
        try {
            this.client = new ElectrumClient(net, false, ELECTRUM_PORT, ELECTRUM_URI, 'tcp')
            await this.client.initElectrum({ client: WALLET_NAME, version: '1.4' });
...
async clearAllWallets(): Promise<void[]> {
    return Promise.all([
        this.wallet.clear(),
        WalletMoneroHandlerModule.deleteAllWallets(),
