  ...
  <div class="dashboard">
    <aside class="sidebar">
      <div :class="['balance', isMac ? 'pushDown' : '']">
        ...
        <p class="balance__amount">
          <span class="balance__amount-large">{{ unlocked_balance_monero || 0 }} / {{ balance_monero || 0 }}</span>
          <span class="balance__currency-abbr">XMR</span>
        </p>
        <p class="balance__amount">
          {{ intlFormater(parseFloat(btcToFiat(balance.toString())) + parseFloat(xmrToFiat(balance_monero.toString()))) }}
      ...
      <nav class="nav">
        <ul class="nav__list">
          ...
          <li>
            <button
              v-on:click="currentTab = 'SendMonero'"
              :class="['nav__btn nav__btn--send', currentTab === 'SendMonero' && 'active']"
            >
              {{ lang.send_xmr }}
            </button>
          </li>
          ...
          <li>
            <button
              v-on:click="currentTab = 'ReceiveXMR'"
              :class="['nav__btn nav__btn--receive', currentTab === 'ReceiveXMR' && 'active']"
            >
              {{ lang.receive_xmr }}
            </button>
          </li>
    ...
    <OverView
      ...
      :transactionsMonero="allTransactionsMonero"
      @show-receive="currentTab = 'Receive'"
      @show-receive-xmr="currentTab = 'ReceiveXMR'"
    ></OverView>
    <Receive
      :style="currentTab == 'Receive' ? '' : 'display: none;'"
      :language="lang"></Receive>
    <ReceiveMonero
      :style="currentTab == 'ReceiveXMR' ? '' : 'display: none;'"
      :language="lang"></ReceiveMonero>
    <Send
    ...
    ></Send>
    <SendMonero
      :style="currentTab == 'SendMonero' ? '' : 'display: none;'"
      :language="lang"
      :lastTransaction="lastTransaction"
      :currency="fiatName"
      @show-amount-modal="showModalMonero()"
      @show-transaction-confirm="showMoneroTransaction"
      @showError="showMoneroError"
    ></SendMonero>
    <Swap
      :style="currentTab == 'Swap' ? '' : 'display: none;'"
      :language="lang"></Swap>
    <Settings
      :style="currentTab == 'Settings' ? '' : 'display: none;'"
      :language="lang"
      @show-seed="showSeed()"
      @show-seed-monero="showSeedMonero()"
      @close="close()"
      @currency-changed="switchCurrency()"
      @language-changed="switchLanguage()"
    ></Settings>
    <!-- modal btc show to send -->
      ...
      <div class="modal modal--send">
        ...
        <div class="send-modal-details-container">
          <div class="send-modal-amount-details">
            ...
            <p class="send-modal-amount-text">0 BTC</p>
            <p class="send-modal-fiat-text">{{ intlFormater(btcToFiat(this.btcAmount)) }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.miner_fee }}</p>
            <p class="send-modal-amount-text">{{ (this.sendingMax ? '-' : '') + this.fee }} BTC</p>
            <p class="send-modal-fiat-text">{{ (this.sendingMax ? '-' : '') + intlFormater(btcToFiat(this.fee)) }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.total }}</p>
            <p class="send-modal-amount-text">{{ this.total }} BTC</p>
            <p class="send-modal-fiat-text">{{ intlFormater(btcToFiat(this.total)) }}</p>
          </div>
          <br>
          <div class="send-modal-amount-details">
            <p>{{ lang.they_receive }}</p>
            <p class="send-modal-amount-text">{{ this.confirmationAmount }} BTC</p>
            <p class="send-modal-fiat-text">{{ intlFormater(btcToFiat(this.confirmationAmount)) }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.recepient }}</p>
    ...
    <!-- monero modal showing -->
    <div class="modal-wrapper" v-if="transactionModalMoneroShowing">
      <div class="modal-overlay"></div>
      <div class="modal modal--send">
        <button v-on:click="hideMoneroTransaction()" class="modal__close">
          <img src="../assets/images/close.svg" alt="close">
        </button>
        <img class="send-modal-icon" src="./../assets/images/send-focus.svg" alt="send">
        <PageTitle>{{ lang.confirmation }}</PageTitle>
        ...
        <div class="send-modal-details-container">
          <div class="send-modal-amount-details">
            <p>{{ lang.amount }}</p>
            <p class="send-modal-amount-text">{{ this.xmr_ammount }} XMR</p>
            
            <p class="send-modal-fiat-text">{{ intlFormater(xmrToFiat(this.xmr_ammount)) }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.miner_fee }}</p>
            <p class="send-modal-amount-text">{{ (this.sendingMax ? '-' : '') + this.fee }} XMR</p>
            <p class="send-modal-fiat-text">{{ (this.sendingMax ? '-' : '') + intlFormater(xmrToFiat(this.fee))  }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.total }}</p>
            <p class="send-modal-amount-text">{{ this.total }} XMR</p>
            <p class="send-modal-fiat-text">{{  intlFormater(xmrToFiat(this.total))}}</p>
          </div>
          <br>
          <div class="send-modal-amount-details">
            <p>{{ lang.they_receive }}</p>
            <p class="send-modal-amount-text">{{ this.confirmationAmount }} XMR</p>
            <p class="send-modal-fiat-text">{{ intlFormater(xmrToFiat(this.confirmationAmount)) }}</p>
          </div>
          <div class="send-modal-amount-details">
            <p>{{ lang.recepient }}</p>
            <p class="send-modal-amount-text" style="color: #F7931A;">{{ this.receivingAddress }}</p>
        ...
        <div class="send-modal-buttons-container">
          ...
          <Checkbox v-if="!sending" class="seed-phrase-checkbox" v-model="thisLooksRight" :label="lang.im_sure"/>
          <ButtonPrimary v-if="!sending" :click="sendTransactionMonero" :disabled="!thisLooksRight">{{ lang.send_xmr }}</ButtonPrimary>
        </div>
        </div>
      </div>
    </div>
    <!-- monero modal error -->
    <div class="modal-wrapper" v-if="errorModalShowing">
      <div class="modal-overlay"></div>
      <div class="modal modal--send">
        <button v-on:click="hideError()" class="modal__close">
          <img src="../assets/images/close.svg" alt="close">
        </button>
        <img class="send-modal-icon" src="./../assets/images/send-focus.svg" alt="send">
        <PageTitle>{{ lang.confirmation }}</PageTitle>
...
<!-- seed mo -->
    ...
    <div class="modal-wrapper" v-if="this.seedModalMonero">
      <div class="modal-overlay"></div>
      <div class="modal">
        <button v-on:click="hideSeedMonero()" class="modal__close">
          <img src="../assets/images/close.svg" alt="close">
        </button>
        <PageTitle>{{ lang.seed_modal_monero }}</PageTitle>
        <PageSubtitle>{{ lang.create_subtitle_2 }}</PageSubtitle>
          
    <div class="ol_parent">
      <div class="ol_container">
        <OL class="list">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[0] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[1] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[2] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[3] }}</span></LI>
        </OL>
      </div>

      <div class="ol_container">
        <OL class="list" start="5">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[4] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[5] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[6] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[7] }}</span></LI>
        </OL>
      </div>

      <div class="ol_container">
        <OL class="list" start="9">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[8] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[9] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[10] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[11] }}</span></LI>
        </OL>
      </div>
      </div>
      <div class="ol_parent" style="margin-top: -55px;">
    </div>
    <br/>
    <div class="ol_parent">
      <div class="ol_container">
        <OL class="list" start="13">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[12] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[13] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[14] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[15] }}</span></LI>
        </OL>
    </div>
    <div class="ol_container">
        <OL class="list" start="17">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[16] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[17] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[18] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[19] }}</span></LI>
        </OL>
    </div>
    <div class="ol_container">
        <OL class="list" start="21">
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[20] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[21] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[22] }}</span></LI>
        <LI><span class="phrase">{{ this.seed_monero.split(' ')[23] }}</span></LI>
        </OL>
    </div>
    </div>
      
      <div class="ol_parent" style="margin-left: -15px; margin-top: -10px;">
      <div class="ol_container">
        <OL class="list" start="25">
          <LI><span class="phrase">{{ this.seed_monero.split(' ')[24] }}</span></LI>
        </OL>
    </div>
    </div>
    </div>
...
import ReceiveMonero from "@/components/ReceiveMonero.vue";
import Send from "@/components/Send.vue";
import SendMonero from "@/components/SendMonero.vue";
...
@Component({ components: { OverView, Receive, ReceiveMonero, Send, SendMonero, Swap, Settings, BackLink, ButtonPrimary, PageTitle, PageSubtitle, Checkbox, OL, LI } })
export default class WalletHomeView extends Vue {
  ...
  private errorModalShowing = false;
  private transactionModalMoneroShowing = false;
  ...
  private transactionsMonero = WalletMoneroHandlerModule.transactions;
  ...
  private balance_monero = new BigNumber(WalletMoneroHandlerModule.balance).dividedBy(1000000000000) || 0;

  private unlocked_balance_monero = new BigNumber(WalletMoneroHandlerModule.unlockedBalance).dividedBy(1000000000000) || 0;
  ...
  private seed_monero = "";
  private path_monero = ""
  private password_monero = ""

  private xmr_ammount: BigNumber = new BigNumber(0);
  private xmr_priority: number = 1
  private xmr_hash: string = "";
  private last_xmr_tx = "";
  ...
  @Prop() language!: Language;
   ...
   get allTransactions() : any[] {
    //all transaction btx + monero concatanated
    ...
    get allTransactionsMonero() {
      return this.transactionsMonero.reverse();
    }
  ...
  async mounted() {
    ...
    let moneroData = await moneroWalletDB.getWallet()
  
    this.seed_monero = moneroData.mnemonic

    this.password_monero = moneroData.password
    this.path_monero = moneroData.path
    ...
    this.balance_monero = new BigNumber(WalletMoneroHandlerModule.balance).dividedBy(1000000000000);

    this.unlocked_balance_monero = new BigNumber(WalletMoneroHandlerModule.unlockedBalance).dividedBy(1000000000000);
  ...
  showSeedMonero() {
    this.seedModalMonero = true;
  }
  hideSeedMonero() {
    this.seedModalMonero = false;
  }
  ...
  async showMoneroTransaction(details: any[]) {
    this.thisLooksRight = false;
    this.sendingMax = details[2];
    this.last_xmr_tx = details[0].tx;
    this.transactionModalMoneroShowing = true;
    this.fee = new BigNumber(details[0].fee)
      .dividedBy(1000000000000)
      .toString();
    this.xmr_ammount = details[3];
    
    this.receivingAddress = details[1].substring(0, 19) + '...' + details[1].substring(85, 95);

    let properAmmount = new BigNumber(this.xmr_ammount).multipliedBy(1000000000000);

    this.total = new BigNumber(details[0].fee).plus(properAmmount).dividedBy(1000000000000).toString();
    

    // The amount the recipient of the transaction gets
    this.confirmationAmount = this.sendingMax
      ? new BigNumber(this.xmr_ammount).minus(this.fee).toString()
      : this.xmr_ammount.toString();

    this.xmr_hash = details[0].tx;
    this.transactionModalMoneroShowing = true;
  }

  showMoneroError(details: any[]) {
    this.last_xmr_tx = details[0].tx;
    this.transactionModalMoneroShowing = true;
  ...
  hideMoneroTransaction() {
    this.transactionModalMoneroShowing = false;
  }
...
// Broadcast the transaction
  ...
  try {
    await WalletMoneroHandlerModule.sendTransaction(this.last_xmr_tx);
  } catch (e) {
    //TODO handle better error, or display it
    console.log(e)
  }
  finally {
    this.sending = false;
    this.hideMoneroTransaction();
    this.last_xmr_tx = "";
    this.xmr_ammount = new BigNumber(0);
    this.xmr_priority = 1;
  ...
  intlFormater(amount: string) {
    return new Intl.NumberFormat(this.walletModule.settings.language, { style: 'currency', currency: this.walletModule.settings.currency }).format(parseFloat(amount));
  }
  ...
  xmrToFiat(xmr: string): string {
    let fiat = (parseFloat(xmr) * WalletMoneroHandlerModule.fiatRate).toFixed(2);
  ...
  async syncWallet( smallSync : boolean) {
    ...
    try {
      ...
      this.transactionsMonero = WalletMoneroHandlerModule.transactions;
      this.unconfirmedTransactions = WalletHandlerModule.unconfirmedTransactions;
      this.balance = WalletHandlerModule.balance
      this.balance_monero = new BigNumber(WalletMoneroHandlerModule.balance).dividedBy(1000000000000) || new BigNumber(0);
      this.unlocked_balance_monero = new BigNumber(WalletMoneroHandlerModule.unlockedBalance).dividedBy(1000000000000) || new BigNumber(0);
