<template>
  <table class="table">
    ...
    <tbody>
      ...
      <tr v-for="tx in transactionsMonero" :key="tx.txid">
          <td>
          ...
          <td>
            <p :class="'amount-' + (zero.gt(tx.amount) ? 'expense' : 'income') ">{{ tx.amount / 1000000000000 }} <span class="amount-currency">XMR</span></p>
            <p class="amount">{{ new Intl.NumberFormat(module.settings.language, { style: 'currency', currency: module.settings.currency }).format(parseFloat(zero.plus(tx.amount).dividedBy(1000000000000).multipliedBy(module.fiatRate).toFixed(2))) }} <span class="amount-currency">{{ module.settings.currency }}</span></p>
          </td>
          <td>
            <a v-on:click="openTransactionView('https://blockchair.com/monero/transaction/' + tx.txid)" class="external-link">
...
import { MoneroTransaction } from '@/wallet/moneroWallet'
...
@Prop() transactionsMonero! : MoneroTransaction[];
private module = WalletMoneroHandlerModule
private zero = new BigNumber(0);
private dateHandler = moment

  getText(tx: any, translate = true) {
    if(tx.isIncoming) {
      return translate ? this.language.incoming_monero : 'Incoming';
    }
    if(tx.isOutgoing) {
      return translate ? this.language.outgoung_monero : 'Outgoing';
    }    
  }
